...
test.describe('Comprehensive Form Validation', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="complex-form"]', { timeout: 10000 });
  });
  ...
  test('should validate cross-field dependencies', async ({ page }) => {
    ...
    const triggerCheckbox = page.locator('[data-testid="enable-conditional-checkbox"]');
    ...
    await expect(page.locator('[data-testid="conditional-error"]')).toBeVisible();
    
    // Fill dependent field
    await conditionalField.fill('Required value');
    await expect(page.locator('[data-testid="conditional-error"]')).not.toBeVisible();
  });
  ...
  test('should handle async validation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="username-validating"]')).toBeVisible();
    
    // Wait for async validation to complete
    await page.waitForSelector('[data-testid="username-validating"]', { state: 'hidden', timeout: 5000 });
  ...
  test('should validate file upload fields', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="avatar-error"]')).toContainText('file type');
    ...
    await expect(page.locator('[data-testid="avatar-error"]')).not.toBeVisible();
  ...
  test('should validate array fields', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="tags-error"]')).toContainText('duplicate');
  ...
  test('should validate complex nested forms', async ({ page }) => {
    ...
    await expect(addressForm.locator('[data-testid="street-error"]')).toBeVisible();
    await expect(addressForm.locator('[data-testid="city-error"]')).toBeVisible();
    await expect(addressForm.locator('[data-testid="zip-error"]')).toBeVisible();
    
    // Fill valid data
    await addressForm.locator('[data-testid="street-input"]').fill('123 Main St');
    await addressForm.locator('[data-testid="city-input"]').fill('Anytown');
    await addressForm.locator('[data-testid="zip-input"]').fill('12345');
    
    // Errors should clear
    await expect(addressForm.locator('[data-testid="street-error"]')).not.toBeVisible();
    await expect(addressForm.locator('[data-testid="city-error"]')).not.toBeVisible();
    await expect(addressForm.locator('[data-testid="zip-error"]')).not.toBeVisible();
  });
  ...
  test('should persist validation state during navigation', async ({ page }) => {
  ...
  test('should handle validation with dynamic field changes', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="dynamic-field-error"]')).toBeVisible();
    ...
    await expect(page.locator('[data-testid="dynamic-field-error"]')).not.toBeVisible();
