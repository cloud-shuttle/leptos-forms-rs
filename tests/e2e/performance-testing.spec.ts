...
test.describe('Performance Testing', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="perf-test-container"]', { timeout: 10000 });
  });
  ...
  test('should handle validation debouncing efficiently', async ({ page }) => {
    ...
    const validationCounter = page.locator('[data-testid="validation-counter"]');
  ...
  test('should handle memory efficiently with form arrays', async ({ page }) => {
  ...
  test('should handle concurrent validations efficiently', async ({ page }) => {
    ...
    for (const p of pages) {
      await expect(p.locator('[data-testid="validation-success"]')).toBeVisible();
    }
  ...
  test('should handle scroll performance with large forms', async ({ page }) => {
    ...
    console.log(`Scroll performance - Bottom: ${scrollToBottomTime}ms, Middle: ${scrollToMiddleTime}ms, Top: ${scrollToTopTime}ms`);
  ...
  test('should maintain performance during error states', async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="error-field-19-error"]', { timeout: 5000 });
    ...
    await expect(page.locator('[data-testid="error-field-19-error"]')).not.toBeVisible();
  ...
  test('should handle memory cleanup properly', async ({ page }) => {
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      await page.fill('[data-testid="dynamic-email"]', `test${iter}@example.com`);
      
      // Destroy form
      await page.click('[data-testid="destroy-dynamic-form"]');
      await expect(page.locator('[data-testid="dynamic-form"]')).not.toBeVisible();
    ...
    console.log(`Average iteration time: ${avgTime}ms, Min: ${minTime}ms, Max: ${maxTime}ms`);
