...
test.describe("Complex Form Example", () => {
  ...
  test("should navigate between form steps", async ({ page }) => {
    ...
    await expect(page.locator('.step[data-step="1"]')).not.toHaveClass(
      /active/,
    );
  ...
  test("should validate each step before allowing progression", async ({
    page,
  }) => {
    // Wait for the first step to be visible
    await page.waitForSelector('.step-content[data-step="1"]', {
    ...
    await page.waitForSelector('.step-content[data-step="2"]', {
      state: "visible",
      timeout: 10000,
    });
  ...
  test("should handle address fields in step 2", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="street-input"]')).toHaveValue(
      "123 Main St",
    );
    await expect(page.locator('[data-testid="city-input"]')).toHaveValue(
      "Anytown",
    );
    await expect(page.locator('[data-testid="zipCode-input"]')).toHaveValue(
      "12345",
    );
  ...
  test("should handle conditional fields", async ({ page }) => {
    ...
    await page.waitForSelector('.step-content[data-step="3"]', {
      state: "visible",
      timeout: 10000,
    });
    ...
    await page.waitForSelector('.step-content[data-step="4"]', {
      state: "visible",
      timeout: 10000,
    });
    ...
    await expect(
      page.locator('[data-testid="newsletter-checkbox"]'),
    ).toBeVisible();
  ...
  test("should handle form submission with all steps completed", async ({
    page,
  }) => {
    // Wait for the first step to be visible
    await page.waitForSelector('.step-content[data-step="1"]', {
