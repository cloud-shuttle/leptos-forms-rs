...
test.describe("Form Library Components", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="component-test-page"]', {
      timeout: 10000,
    });
  });
  ...
  test("should render TextInput component correctly", async ({ page }) => {
    ...
    // Check label
    await expect(page.locator('label[for="test-text"]')).toContainText(
      "Test Text Input",
    );
  ...
  test("should render SelectInput component correctly", async ({ page }) => {
    ...
    // Check options
    await expect(select.locator('option[value="option1"]')).toContainText(
      "Option 1",
    );
    await expect(select.locator('option[value="option2"]')).toContainText(
      "Option 2",
    );
    await expect(select.locator('option[value="option3"]')).toContainText(
      "Option 3",
    );
  ...
  test("should handle field arrays correctly", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="field-array-1"]'),
    ).not.toBeVisible();
  });
  ...
  test("should handle form reset correctly", async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="number-input"]')).toHaveValue("");
    await expect(
      page.locator('[data-testid="checkbox-input"]'),
    ).not.toBeChecked();
