...
test.describe('Test Helpers', () => {
  test('should provide common form selectors', () => {
    ...
    expect(FormSelectors.VALIDATION_ERROR).toBe('[data-testid="validation-error"]');
    expect(FormSelectors.ERROR_MESSAGE).toBe('[data-testid="error-message"]');
    expect(FormSelectors.SUCCESS_MESSAGE).toBe('[data-testid="success-message"]');
    ...
    expect(FormSelectors.SHOW_CONDITIONAL).toBe('[data-testid="show-conditional"]');
    expect(FormSelectors.CONDITIONAL_FIELD).toBe('[data-testid="conditional-field"]');
  });
