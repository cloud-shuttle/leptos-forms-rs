    ...
    fn field_metadata() -> Vec<leptos_forms_rs::core::FieldMetadata> {
        vec![
            leptos_forms_rs::core::FieldMetadata {
                ...
                field_type: leptos_forms_rs::core::types::FieldType::Select(vec![
            ...
            "business" => {
                    ...
                    .map_or(true, |s| s.trim().is_empty())
                {
                    errors.add_field_error(
                        "company_name",
                        "Company name is required for business accounts".to_string(),
                    );
                }
                if self.tax_id.as_ref().map_or(true, |s| s.trim().is_empty()) {
                    errors.add_field_error(
                        "tax_id",
                        "Tax ID is required for business accounts".to_string(),
                    );
                }
            }
            "personal" => {
                    ...
                    .map_or(true, |s| s.trim().is_empty())
                {
                    errors.add_field_error(
                        "personal_id",
                        "Personal ID is required for personal accounts".to_string(),
                    );
                }
            }
            _ => {
...
fn test_conditional_validation_personal_account() {
    ...
    assert!(
        result.is_ok(),
        "Personal account should be valid with required fields"
    );
}
...
fn test_conditional_validation_business_account() {
    ...
    assert!(
        result.is_ok(),
        "Business account should be valid with required fields"
    );
}
...
fn test_conditional_validation_business_missing_fields() {
    ...
    assert!(
        result.is_err(),
        "Business account should fail validation without company name"
    );

    if let Err(errors) = result {
        assert!(
            errors.has_field_error("company_name"),
            "Should have company_name error"
        );
        assert!(
            !errors.has_field_error("personal_id"),
            "Should not have personal_id error for business account"
        );
    }
}
...
fn test_conditional_validation_personal_missing_fields() {
    ...
    assert!(
        result.is_err(),
        "Personal account should fail validation without personal ID"
    );

    if let Err(errors) = result {
        assert!(
            errors.has_field_error("personal_id"),
            "Should have personal_id error"
        );
        assert!(
            !errors.has_field_error("company_name"),
            "Should not have company_name error for personal account"
        );
    }
}
...
fn test_conditional_validation_invalid_account_type() {
    ...
    if let Err(errors) = result {
        assert!(
            errors.has_field_error("account_type"),
            "Should have account_type error"
        );
    }
}
...
fn test_field_dependencies_metadata() {
    ...
    assert!(company_name_field
        .dependencies
        .contains(&"account_type".to_string()));
    ...
    assert!(personal_id_field
        .dependencies
        .contains(&"account_type".to_string()));

    let email_field = metadata.iter().find(|f| f.name == "email").unwrap();
    assert!(
        email_field.dependencies.is_empty(),
        "Email should have no dependencies"
    );
}
