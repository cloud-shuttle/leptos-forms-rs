use leptos::prelude::*;
use leptos_forms_rs::core::{FieldMetadata, FieldType, FieldValue};
...
impl Form for EnhancedFieldArrayForm {
    ...
    fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        for (index, contact) in self.contacts.iter().enumerate() {
            if contact.name.is_empty() {
                errors.add_field_error(
                    &format!("contacts[{}].name", index),
                    "Contact name is required".to_string(),
                );
            }
            if !contact.email.contains('@') {
                errors.add_field_error(
                    &format!("contacts[{}].email", index),
    ...
    fn get_field_value(&self, field_name: &str) -> FieldValue {
        match field_name {
            ...
            "nested_arrays" => FieldValue::Array(
                self.nested_arrays
                    .iter()
                    .map(|arr| {
                        FieldValue::Array(
                            arr.iter().map(|s| FieldValue::String(s.clone())).collect(),
                        )
                    })
                    .collect(),
            ),
            "contacts" => FieldValue::Array(
                self.contacts
                    .iter()
                    .map(|contact| {
                        let mut obj = HashMap::new();
                        ...
                        obj.insert(
                            "email".to_string(),
                            FieldValue::String(contact.email.clone()),
                        );
                        obj.insert(
                            "phones".to_string(),
                            FieldValue::Array(
