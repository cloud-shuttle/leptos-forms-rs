    ...
    fn validate(&self) -> Result<(), ValidationErrors> {
        ...
        if !self.email_field.is_empty() {
            if !self.email_field.contains('@') || !self.email_field.contains('.') {
                errors.add_field_error("email_field", "Please enter a valid email address".to_string());
            }
        } else {
        ...
        if !self.url_field.is_empty() {
            if !(self.url_field.starts_with("http://") || self.url_field.starts_with("https://")) {
                errors.add_field_error("url_field", "URL must start with http:// or https://".to_string());
            }
        }
        ...
        if !self.phone_field.is_empty() {
            ...
            if !phone_regex.is_match(&self.phone_field) {
                errors.add_field_error("phone_field", "Please enter a valid phone number".to_string());
            }
        }
        
        // Length validations
        if !self.min_length_field.is_empty() && self.min_length_field.len() < 5 {
            errors.add_field_error("min_length_field", "Must be at least 5 characters".to_string());
        }
        
        if !self.max_length_field.is_empty() && self.max_length_field.len() > 10 {
            errors.add_field_error("max_length_field", "Must be at most 10 characters".to_string());
        }
        
        if !self.exact_length_field.is_empty() && self.exact_length_field.len() != 8 {
            errors.add_field_error("exact_length_field", "Must be exactly 8 characters".to_string());
        }
        ...
        if !self.regex_field.is_empty() {
            ...
            if !pattern_regex.is_match(&self.regex_field) {
                errors.add_field_error("regex_field", "Must match format: 2-4 uppercase letters followed by 3-6 digits".to_string());
            }
        }
        ...
        if !self.alphanumeric_field.is_empty() {
            ...
            if !alphanumeric_regex.is_match(&self.alphanumeric_field) {
                errors.add_field_error("alphanumeric_field", "Must contain only letters and numbers".to_string());
            }
        }
        
        // Conditional validation
        if self.dependency_trigger && self.conditional_required.is_empty() {
            errors.add_field_error("conditional_required", "This field is required when dependency is enabled".to_string());
        }
        
        // Custom validation example
        if !self.custom_validation_field.is_empty() {
            if self.custom_validation_field.to_lowercase() == "forbidden" {
                errors.add_field_error("custom_validation_field", "This value is not allowed".to_string());
            }
        }
