use leptos::prelude::*;
use crate::core::{Form, FieldValue};
use crate::components::form_wizard::{FormWizard, WizardStep};
use crate::hooks::{use_form, use_field_array};
...
impl Form for SimpleTestForm {
    fn field_metadata() -> Vec<crate::core::FieldMetadata> where Self: Sized {
        vec![
    ...
    fn get_field(&self, name: &str) -> Option<FieldValue> {
        match name {
            ...
            "tags" => Some(FieldValue::Array(
                self.tags.iter().map(|t| FieldValue::String(t.clone())).collect()
            )),
            _ => None,
        }
    }
    ...
    fn schema() -> crate::core::FormSchema where Self: Sized {
